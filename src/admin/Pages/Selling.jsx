...
function Selling() {
  ...
  const [showMoreIndustrialBuilding, setShowMoreIndustrialBuilding] =
    useState(false);
  const [showMoreLand, setShowMoreLand] = useState(false);
  const [showMoreShowroom, setShowMoreShowroom] = useState(false);
  const [showMoreServiceApartment, setShowMoreServiceApartment] =
    useState(false);
    ...
    <div className="row" style={{ marginBottom: "10%" }}>
      {plotData && (
        <div className="col">
          <div
            className="card "
            style={{ margin: "1%", borderColor: "red", width: "98%" }}
          >
            ...
            <h6 className="mb-5 mt-5 p-2">
              3 BHK Flat for Sale in Perungudi, Chennai
            </h6>

            {/* Plot */}
            {plotData?.property_type === "plot" && (
              <div className="d-flex flex-wrap gap-3 mx-4">
                {plotData?.plot_properties?.plot_images?.map((img, index) => (
                  <img
                    src={img.image}
                    alt={`Image ${index + 1}`}
                    style={
            ...
            <div className="row p-1">
              {plotData.plot_properties?.plot_images
              ...
              {/* image 2 */}
              {houseData?.residential_properties?.house?.house_images
                ?.slice(0, showMore ? undefined : 1)
                .map((imageObj, index) => (
              ...
              {houseData?.residential_properties?.house?.house_images?.length >
                1 && (
                <span
                  onClick={() => setShowMore(!showMore)}
                  className="text-primary"
              ...
              {/* image 3 completed */}
              {landData.land_properties?.land_images
                ?.slice(0, showMoreLand ? undefined : 1)
                .map((imageObj, index) => (
              ...
              {landData.land_properties?.land_images?.length > 0 && (
                <span
                  onClick={() => setShowMoreLand(!showMoreLand)}
                  className="text-primary"
              ...
              {/* image 4  completed*/}
              {serviceApartmentData.commercial_properties?.service_apartment?.service_apartment_images
                ?.slice(0, showMoreServiceApartment ? undefined : 1)
                .map((imageObj, index) => (
                  <div key={`service-apartment-${index}`} className="col-lg-3">
                    <img
                      src={imageObj.image}
                      alt={`Service Apartment ${index}`}
                      style={{
                        width: "100%",
                        height: "500px",
                        marginTop: "1%",
              ...
              {serviceApartmentData.commercial_properties?.service_apartment
                ?.service_apartment_images?.length > 1 && (
                <span
                  onClick={() =>
                    setShowMoreServiceApartment(!showMoreServiceApartment)
                  }
                  className="text-primary"
                  style={{ textAlign: "center" }}
                >
                  {showMoreServiceApartment
                    ? "Show Less Images"
                    : "View More Images"}
                </span>
              )}
              {/* image 5 */}
              {factoryData.commercial_properties?.factory?.factory_images
                ?.slice(0, showMoreFactory ? undefined : 1)
                .map((imageObj, index) => (
              ...
              {factoryData.commercial_properties?.factory?.factory_images
                ?.length > 1 && (
                <span
                  onClick={() => setShowMoreFactory(!showMoreFactory)}
                  className="text-primary"
                  style={{ textAlign: "center" }}
                >
              ...
              {industrialBuildingData.commercial_properties?.industrialbuilding?.industrialbuilding_images
                ?.slice(0, showMoreIndustrialBuilding ? undefined : 1)
                .map((imageObj, index) => (
                  <div
                    key={`industrial-building-${index}`}
                    className="col-lg-3"
                  >
                    <img
                      src={imageObj.image}
                      alt={`Industrial Building ${index}`}
                      style={{
                        width: "100%",
                        height: "500px",
                        marginTop: "1%",
              ...
              {industrialBuildingData.commercial_properties?.industrialbuilding
                ?.industrialbuilding_images?.length > 1 && (
                <span
                  onClick={() =>
                    setShowMoreIndustrialBuilding(!showMoreIndustrialBuilding)
                  }
                  className="text-primary"
                  style={{ textAlign: "center" }}
                >
                  {showMoreIndustrialBuilding
                    ? "Show Less Images"
                    : "View More Images"}
                </span>
              )}
              ...
              {showroomData.commercial_properties?.showroom?.showroom_images
                ?.slice(0, showMoreShowroom ? undefined : 1)
                .map((imageObj, index) => (
                  <div key={`showroom-${index}`} className="col-lg-3">
                    <img
                      src={imageObj.image}
              ...
              {showroomData.commercial_properties?.showroom?.showroom_images
                ?.length > 1 && (
                <span
                  onClick={() => setShowMoreShowroom(!showMoreShowroom)}
                  className="text-primary"
                  style={{ textAlign: "center" }}
                >
              ...
              {apartmentData?.residential_properties?.apartment?.apartment_images
                ?.slice(0, showMore ? undefined : 1)
                .map((imageObj, index) => (
              ...
              {apartmentData?.residential_properties?.apartment
                ?.apartment_images?.length > 1 && (
                <span
                  onClick={() => setShowMore(!showMore)}
                  className="text-primary"
              ...
              {properties.commercial_properties?.pg_colony && (
                <div className="row p-1">
                  {properties.commercial_properties.pg_colony.pgcolony_images
                    ?.slice(0, showMore ? undefined : 1)
                    .map((imageObj, index) => (
            ...
            <table className="table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {" "}
                    <h6>Estimated EMI: ₹73966</h6>
                    <p style={{ fontSize: "10px" }}>Apply for Home Loan</p>
                  </td>
                </tr>
                <tr>
                  <td>
          ...
          <div className="row">
            <div className="col d-flex justify-content-end">
              <button
                ...
                style={{ width: "150px", margin: "3%" }}
                onClick={() => approvereject("reject", id, setIsReject)}
              >
                Reject
              </button>
              <button
                ...
                style={{ width: "150px", margin: "3%" }}
                onClick={() => approvereject("approve", id, setIsApproved)}
              >
                Approve
              </button>
            </div>
