...
const Pgrent = ({ activeButton, user, second, selectedPropType }) => {
  ...
  const handleRemoveOccupay = (Removeoccupay) => {
    if (newOccupay.includes(Removeoccupay)) {
      setButtonoccupay((prevOccupayy) => ({
        ...prevOccupayy,
        [Removeoccupay]: false,
      }));
      setOccupayy((prevOccupayy) =>
        prevOccupayy.filter((item) => item !== Removeoccupay)
      );
    } else {
      setOccupayy((prevOccupayy) =>
        prevOccupayy.filter((item) => item !== Removeoccupay)
  ...
  const submitForm = async (formValue) => {
    ...
    formData.append("pgcolony.no_of_rooms", parseInt(formValue?.numberofrooms));
    ...
    formData.append(
      "pgcolony.single_room_price_for_ac",
      formValue?.singleroomac
    );
    formData.append(
      "pgcolony.single_room_price_for_nonac",
      formValue?.singleroomnonac
    );
    formData.append(
      "pgcolony.single_room_price_per_month",
      formValue?.singleroompermonth
    );
    formData.append(
      "pgcolony.double_room_price_for_ac",
      formValue?.doubleroomac
    );
    formData.append(
      "pgcolony.double_room_price_for_nonac",
      formValue?.doubleroomnonac
    );
    formData.append(
      "pgcolony.double_room_price_per_month",
      formValue?.doubleroompermonth
    );
    formData.append(
      "pgcolony.triple_room_price_for_ac",
      formValue?.multisharingroomac
    );
    formData.append(
      "pgcolony.triple_room_price_for_nonac",
      parseInt(formValue?.multisharingroomnonac)
    );
    formData.append(
      "pgcolony.triple_room_price_per_month",
      formValue?.multisharingroompermonth
    );
    ...
    <div>
      ...
      <div className="custom-radio d-flex mt-4" onChange={handleChange}>
        ...
        <div className="flex-grow-1 me-2">
          <input
            className="inp"
            placeholder="If existing, Age of the property..."
            onChange={handleChange}
            name="age"
          />
        </div>
              ...
              <div className="d-flex flex-lg-row gap-3">
                {Object.keys(buttonoccupay).map((typess) => (
                  <div key={typess} className="mb-2">
                    <button
                      className={`btn ${
                        buttonoccupay[typess] ? "btn-Nam" : "btn-Name"
                      }`}
            ...
            <Form.Group controlId="formGroup3">
              ...
              <div className="">
                <Form.Control
                  ...
                  // onChange={handleChange}
                >
                  <option value="1bhk">Per day</option>
                  <option value="1bhk">Per Month</option>
                  <option value="1bhk">Per Year</option>
                </Form.Control>
              </div>
